...
func TestCheckSendsStoredCredentials(t *testing.T) {
	...
	creds := staticStore{"": Credentials{Username: "alice", Password: "s3cret"}}
	c, _ := newChecker(t, f, creds)

	got := checkOne(t, c, Image{Ref: "git.example.de/acme/app:abap", LocalDigest: localDigest})
	if got.State != StateCurrent {
		t.Fatalf("State = %v (%s), want current", got.State, got.Reason)
	}
	if !f.gotBasicAuth {
...
func TestDockerConfigStoreLookup(t *testing.T) {
	encoded := base64.StdEncoding.EncodeToString([]byte("alice:t0ken"))
	path := writeDockerConfig(t, fmt.Sprintf(`{
	  "auths": {
	    "git.example.de": {"auth": %q},
	...
	t.Run("base64 auth field", func(t *testing.T) {
		got := store.Lookup("git.example.de")
		if got.Username != "alice" || got.Password != "t0ken" {
			t.Errorf("got %+v, want alice/t0ken", got)
		}
		if !got.Usable() {
